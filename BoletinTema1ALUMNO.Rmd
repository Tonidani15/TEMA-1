...
author: "Antonio Hernando Graboleda"
date:  "16/02/2022"  #Pondría la fecha del día actual
params:
  lang: ES
...
NAME <- rownames(mammals)
NAME
colnames(mammals)

NAME2 <- rownames(Animals2)
NAME2
colnames(Animals2)
...
```{r}
commonAnimals <- intersect(NAME2,NAME)
commonAnimals
```
...
```{r}
ANIMAL <- setdiff(rownames(DF), rownames(DF2))
ANIMAL

ANIMAL2 <- setdiff(rownames(DF2), rownames(DF))
ANIMAL2
...
```{r}
# Ambos dataframes guardan animales, pero la diferencia que podemos contemplar es que en cada uno de los data frames, el mismo animal, presenta nombres distintos.
...
# Habrá 221 librerias instaladas
